...
A general cognitive factor, \(g\), was estimated from ten cognitive indicators. The indicators were z-scored across complete cases.
...
@{}}
\toprule
\textbf{Model and scope} & \textbf{Definition} \\
\midrule
\texttt{PureBinaryMOP}\\
\emph{Full transition row}
& Recursive value model over binary transition support, without a destination cost \\
\addlinespace[4pt]

\texttt{DispersionDestinationMOP}\\
\emph{Full transition row}
& Full-row MOP model with a destination-dispersion cost \\
\addlinespace[4pt]

\texttt{OffdiagPureMOP}\\
\emph{Switch destinations only}
& MOP policy over switch destinations, with self-transitions removed \\
\addlinespace[4pt]

\texttt{OffdiagDispersion}\\
\texttt{DestinationMOP}\\
\emph{Switch destinations only}
& Switch-only MOP model with a destination-dispersion cost \\
\addlinespace[4pt]
...
\begin{figure}[H]
\centering
\includegraphics[width=0.88\textwidth,height=0.58\textheight,keepaspectratio]{figures/appendix/a01_state_atlas.png}
