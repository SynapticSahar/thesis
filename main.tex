...
participant contributed 900 volumes sampled with \(\mathrm{TR} = 1.0\,\mathrm{s}\). The analysis was
...
No participant was excluded on data-quality grounds. Motion and nuisance records were not available alongside the parcel time series, so acquisition-level quality control could not be repeated in the present analysis. Thirty-two participants showed statistical phenotype extremes, including 12 severe statistical extremes. None was an independently identified acquisition-quality failure. These participants were retained in the main analysis and used in sensitivity analyses. One participant was also designated as the primary single-case sensitivity check. Each run of the analysis was stored with a timestamp, a checksum manifest of its outputs, and a record of the computing environment, allowing the results to be reproduced exactly.
...
The mixing parameter was fixed at $\alpha = 0.01$, making the model ridge-dominant while still allowing weak sparsity. In the current ridge implementation, predictors and outcomes were globally standardized before outer cross-validation. Model fitting and penalty selection occurred inside training folds. The analysis is therefore conditional and not fully fold-local. Model performance was evaluated using 100 repeats of 10-fold cross-validation. The reported cross-validated correlation and coefficient of determination therefore describe how well the regularized model explains cognitive variation conditional on the full-sample feature mask. They should not be interpreted as fully unbiased estimates of external predictive performance.
...
The control analysis confirmed the importance of removing the global synchronization level before clustering. Without this correction, the resulting atlas differed strongly from the primary atlas. State labels accounted for 25.2\% of the variance in mean local synchronization in the primary atlas, compared with 87.8\% in the uncorrected atlas. Thus, the primary atlas mainly captured spatial configuration of local synchronization rather than overall synchronization amplitude.
...
The nonlinear comparison has not been rerun using the latest transition features, masks, and ridge baseline. The preceding nonlinear results are therefore not reported, and no current nonlinear-versus-linear conclusion is drawn.
