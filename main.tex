...
\begin{figure}[htbp]
\centering
\includegraphics[width=\textwidth,height=0.72\textheight,keepaspectratio]{figures/appendix/a01_state_atlas.png}
\caption{\textbf{Seven-state synchronization atlas and clustering diagnostics.} The upper panels show k-means inertia, average silhouette width, and the number of volumes assigned to each state. The lower panels show the seven centroid profiles in raw local synchronization space and the corresponding labels projected onto the first two principal components. The seven-state solution is used as a predefined working atlas rather than as an optimal discrete partition.}
\label{fig:app-state-atlas}
\end{figure}

\begin{figure}[p]
\centering
\includegraphics[width=\textwidth,height=0.72\textheight,keepaspectratio]{figures/appendix/a02_state_dynamics.png}
\caption{\textbf{State assignments and participant-level dynamic summaries.} Example state sequences and distances from the assigned centroid are shown alongside participant occupancies, mean state occupancy, the pooled transition matrix, the distribution of switch rates, and state-specific dwell-time distributions. The strong transition-matrix diagonal reflects persistent runs within states.}
\label{fig:app-state-dynamics}
\end{figure}

\begin{figure}[p]
\centering
\includegraphics[width=\textwidth,height=0.72\textheight,keepaspectratio]{figures/appendix/a03_gmm_validation.png}
\caption{\textbf{Gaussian-mixture validation of the synchronization state space.} Model-selection and participant-blocked predictive-density curves are shown with a single-Gaussian diagnostic, low-dimensional projections, k-means--mixture overlap, posterior assignment confidence, and mixture weights. The diagnostics indicate a complex, overlapping density for which the available criteria do not uniquely support seven discrete components.}
\label{fig:app-gmm-validation}
\end{figure}

\begin{figure}[p]
\centering
\includegraphics[width=\textwidth,height=0.72\textheight,keepaspectratio]{figures/appendix/a04_gmm_overlap_k7.png}
\caption{\textbf{Overlap between the seven-state atlas and a seven-component Gaussian mixture.} Raw and normalized cross-tabulations compare k-means state labels with Gaussian-mixture assignments. The low adjusted Rand index shows that the mixture components do not recover the predefined seven-state partition, despite generally high posterior assignment confidence.}
...
\begin{figure}[htbp]
\centering
\includegraphics[width=\textwidth,height=0.72\textheight,keepaspectratio]{figures/appendix/a05_surrogate_nulls.png}
\caption{\textbf{Occupancy- and dwell-preserving surrogate-null analyses.} The upper panels compare the pooled observed transition entropy with each surrogate distribution. The middle and lower panels show participant-level observed-versus-null values, observed-minus-null differences, and the number of participants with lower-tail corrected results. Lower observed entropy indicates temporal structure beyond the constraints preserved by the corresponding null.}
\label{fig:app-surrogate-nulls}
\end{figure}

\begin{figure}[p]
\centering
\includegraphics[width=\textwidth,height=0.72\textheight,keepaspectratio]{figures/appendix/a06_surrogate_signflip.png}
\caption{\textbf{Participant-level sign-flip tests of transition-entropy differences.} Null distributions of the mean participant-level entropy difference are shown for the full and off-diagonal entropy measures under occupancy-preserving and dwell-preserving surrogates. Red lines mark the observed mean differences; negative values indicate lower observed entropy than under the null.}
\label{fig:app-surrogate-signflip}
\end{figure}

\begin{figure}[p]
\centering
\includegraphics[width=\textwidth,height=0.72\textheight,keepaspectratio]{figures/appendix/a07_geometry_group_fit.png}
\caption{\textbf{Full-sample graded-geometry fit.} Predicted and empirical transition probabilities are shown separately for diagonal and off-diagonal entries. The exponential distance law captures the ordering of off-diagonal switch probabilities but does not reproduce the magnitude or across-state pattern of self-transition probabilities.}
\label{fig:app-geometry-group}
\end{figure}

\begin{figure}[p]
\centering
\includegraphics[width=\textwidth,height=0.72\textheight,keepaspectratio]{figures/appendix/a08_geometry_loocv.png}
\caption{\textbf{Held-out graded-geometry fit across participants.} Distributions of leave-one-participant-out correlations are shown for off-diagonal switch routing and diagonal persistence. The off-diagonal distribution is consistently positive, whereas diagonal correlations are broad and centred near zero.}
\label{fig:app-geometry-loocv}
\end{figure}

\begin{figure}[p]
\centering
\includegraphics[width=\textwidth,height=0.72\textheight,keepaspectratio]{figures/appendix/a09_geometry_cognition.png}
\caption{\textbf{Associations between graded-geometry fit and cognition.} Cells show partial Pearson correlations between participant-level held-out fit and cognitive measures after adjustment for age and sex. Effects were weak, and none survived false-discovery-rate correction.}
\label{fig:app-geometry-cognition}
\end{figure}

\begin{figure}[p]
\centering
\includegraphics[width=\textwidth,height=0.72\textheight,keepaspectratio]{figures/appendix/a10_dwell_bias.png}
\caption{\textbf{Global-stay and state-specific stay models.} Empirical and fitted transition matrices are shown with state-specific stay probabilities and full-sample and held-out performance summaries. Allowing the stay probability to vary across states improves persistence modelling while leaving conditional switch routing uniform.}
\label{fig:app-dwell-bias}
\end{figure}

\begin{figure}[p]
\centering
\includegraphics[width=\textwidth,height=0.72\textheight,keepaspectratio]{figures/appendix/a11_binary_mop.png}
\caption{\textbf{Binary-action maximum occupancy comparison.} The pure binary-support MOP baseline is uniform because the observed action support is saturated. Adding the retained destination-dispersion cost creates destination-dependent probabilities and changes full-row and diagonal fit, but provides little off-diagonal routing accuracy.}
\label{fig:app-binary-mop}
\end{figure}

\begin{figure}[p]
\centering
\includegraphics[width=\textwidth,height=0.72\textheight,keepaspectratio]{figures/appendix/a12_stay_switch_mop.png}
\caption{\textbf{Stay/switch decomposition and off-diagonal MOP comparison.} The upper panels separate empirical persistence from conditional switch routing and compare the topology-only and dispersion-cost switch policies. The lower panels summarize full-sample fit and held-out improvements. State-specific stay gates account for persistence, while the destination-dispersion term yields only a small improvement in conditional switch routing.}
...
\begin{figure}[htbp]
\centering
\includegraphics[width=\textwidth,height=0.72\textheight,keepaspectratio]{figures/appendix/a13_entropy_scatterplots.png}
\caption{\textbf{Leading normalized entropy--cognition associations.} Scatterplots show the six highest-ranked pairs from the primary permutation analysis, with ordinary least-squares trend lines. The highlighted participant is the prespecified single-case sensitivity observation. Reported $q$-values are false-discovery-rate corrected across the complete entropy-by-cognition family.}
\label{fig:app-entropy-scatters}
\end{figure}

\begin{figure}[p]
\centering
\includegraphics[width=\textwidth,height=0.72\textheight,keepaspectratio]{figures/appendix/a14_entropy_partial_sensitivity.png}
\caption{\textbf{Contextual sensitivity of entropy--cognition associations.} Unadjusted correlations are compared with partial correlations controlling for global synchronization and metastability, switch rate and maximum state occupancy, or all contextual measures. The leading associations are generally attenuated after adjustment, especially when switch rate and occupancy are included.}
\label{fig:app-entropy-partial}
\end{figure}

\begin{figure}[p]
\centering
\includegraphics[width=\textwidth,height=0.72\textheight,keepaspectratio]{figures/appendix/a15_entropy_cognition_heatmap.png}
\caption{\textbf{Normalized entropy--cognition association matrix.} Cells report Pearson correlations for the four normalized entropy measures and seven cognitive outcomes. A plus sign denotes an uncorrected permutation result below 0.05; no cell survived false-discovery-rate correction across the full family.}
